...
export function buildActionList(context: ActionContext): string {
  ...
  actions.push('- build_deliverable: Build a complete first draft (Three Tier + Five Chapter Story) from an existing offering and audience. Runs the full pipeline autonomously — mapping, message generation, story writing, voice check, polishing. Takes a few minutes. Params: { offeringName: string, audienceName: string, medium: string, situation?: string, verbatimAsk?: string } — medium options: email, blog, landing_page, in_person, press_release, newsletter, one-pager, report, pitch_deck. situation is the specific context or occasion for this deliverable (e.g., "Q3 partnership webinar invitation", "investor meeting next week"). verbatimAsk is the EXACT WORDING the user used to state what they want the audience to do — pull the literal sentence verbatim from the user\'s input, do NOT paraphrase, do NOT shorten, do NOT generalize. Strip leading "I want them to" / "the ask is" / "tell them to" only if it leaves a clean imperative (e.g. user said "I want them to confirm participation in our joint Q3 webinar by May 15" → verbatimAsk = "confirm participation in our joint Q3 webinar by May 15"). Tone notes ("the tone should be partner-to-partner") are NOT asks — skip them. If the user did not state an ask, omit verbatimAsk entirely (empty is better than fabricated).');
