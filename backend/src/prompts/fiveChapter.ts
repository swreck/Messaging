...
export function buildChapterPrompt(chapterNum: number, medium?: string, emphasisChapter?: number, sourceContent?: { medium: string; chapterText: string }): string {
  ...
  // Bundle 1B Commit D — EMAIL_ENVELOPE_BOUNDARY removed. Replaced by the
  // per-chapter format directive in mediums.ts:email.perChapterFormat,
  // which is read upstream of chapterRules in formatGuidance. The per-
  // chapter format directive ("Body prose only. Do NOT emit Subject,
  // salutation, or signoff") is now the canonical instruction; competing
  // it with a separate guardrail produced no signal Maria could honor
  // unambiguously. Walk B Item 5 Site 4 root cause closed structurally.
...
- ANTI-INVENTION ON COMMERCIAL TERMS: do NOT invent commercial offers, evaluation periods, refund policies, free-trial durations, money-back guarantees, pilot terms, pricing concessions, demo-call lengths, or any other commercial term the user did not supply. If the user said "I want them to take a 20-minute meeting," you can write "20-minute meeting" — that's their term. You may NOT add "and we offer a 30-day evaluation with a full refund" unless the user named that. If the chapter would benefit from a commercial term and the user didn't supply one, emit \`[INSERT: <one-sentence description in user's voice of what offer goes here>]\` instead. Examples: \`[INSERT: the evaluation, trial, or first-step offer you want to put on the table]\`, \`[INSERT: how much of your time you're willing to commit to this first conversation]\`.`,
  ...
  // Bundle 1B Commit D — per-chapter format directive. When a medium provides
  // perChapterFormat[chapterNum], the chapter prompt receives ONLY that
  // chapter's piece of the format spec. The artifact-level `format` string
  // (envelope shape: salutation/sign-off/headline/dateline) is read by the
  // BLEND layer when composing around the joined chapters. Avoids the
  // structural mismatch where the per-chapter prompt and the chapter-level
  // guardrail contradicted each other (Walk B Item 5 Site 4 root cause).
  // Fallback: when perChapterFormat is undefined for a medium, the old
  // behavior applies (artifact-level format injected into every chapter).
  const formatDirective = spec
    ? (spec.perChapterFormat && spec.perChapterFormat[chapterNum as 1 | 2 | 3 | 4 | 5]
        ? spec.perChapterFormat[chapterNum as 1 | 2 | 3 | 4 | 5]
        : spec.format)
    : '';
  const formatGuidance = spec ? `
CONTENT FORMAT: ${spec.label}
FORMAT RULES: ${formatDirective}
