...
9. NO CONTRAST CLAUSES — this is critical. Never write "not X," "instead of X," "no X," "without the X," "rather than X" after stating a fact. State what you DO, what IS true, what WORKS. Stop. The reader knows their current situation — you don't need to position against it. Example: write "audit prep takes weeks" not "audit prep takes weeks instead of months." Write "it connects to your existing stack" not "no rip-and-replace." This applies to EVERY chapter, including section headers.
